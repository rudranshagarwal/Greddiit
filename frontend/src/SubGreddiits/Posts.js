...
export default function Posts() {
    ...
    function handleSubmitcomment(event) {
        ...
        setiscomment(false)
        const token = localStorage.getItem('token')
        axios.post('http://localhost:3001/api/SubGreddiit/newcomment', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function Report(event) {
        ...
        setshowreport(false)
        const token = localStorage.getItem('token')
        axios.post('http://localhost:3001/api/SubGreddiit/Report', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function upvote(post, event) {
        const token = localStorage.getItem('token')
        axios.post('http://localhost:3001/api/SubGreddiit/upvote', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function downvote(post, event) {
        const token = localStorage.getItem('token')
        axios.post('http://localhost:3001/api/SubGreddiit/downvote', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    useEffect(() => {
        const token = localStorage.getItem('token')
        axios.get("http://localhost:3001/api/SubGreddiit/getinfo", {
            headers:
            {
                Authorization: `Bearer ${token}`
            }
