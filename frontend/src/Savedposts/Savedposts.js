...
export default function Posts() {
    ...
    function handleSubmitcomment(event) {
        ...
        setiscomment(false)
        const token = localStorage.getItem('token')
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/newcomment', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function Report(event) {
        ...
        setshowreport(false)
        const token = localStorage.getItem('token')
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/Report', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function upvote(post, event) {
        const token = localStorage.getItem('token')
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/upvote', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function downvote(post, event) {
        const token = localStorage.getItem('token')
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/downvote', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params: {
    ...
    function unsavepost(post, event) {
        ...
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/unsavepost', {
            ...
            setsavedposts(savedposts.filter(value => value._id != post._id))
        }).catch(error =>
            console.log(error.message))
    }
    ...
    useEffect(() => {
        const token = localStorage.getItem('token')
        axios.get("https://greddiit-backend-l5kk.onrender.com/api/getsavedposts", {
            headers:
            {
                Authorization: `Bearer ${token}`
            }
                ...
                return (
                    <div className="Postdisplay">
                        ...
                        <div className="Postsextra">
                            ...
                            <a onClick={follow.bind(this, post)} style={{ visibility: post.postedby === username ? "hidden" : "visible " }}>Follow </a>
                            <img onClick={upvote.bind(this, post)} className="Upvote" src="https://img.icons8.com/dusk/64/null/facebook-like.png" style={{ visibility: post.upvotes?.includes(username) ? "hidden" : "visible" }} />
                            <img onClick={downvote.bind(this, post)} className="Downvote" src="https://img.icons8.com/dusk/64/null/thumbs-down.png" style={{ visibility: post.downvotes?.includes(username) ? "hidden" : "visible" }} />
    ...
    if (savedposts) {
        ...
        posts = savedposts.map(currpost => { return <Postdisplay post={currpost} /> })
    }
