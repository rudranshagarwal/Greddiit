...
export default function Reported() {
    ...
    if (counter <= 0 && displayblock) {
        ...
        axios.post('http://localhost:3001/api/block', {
            headers: {
    ...
    useEffect(() => {
        const token = localStorage.getItem('token')
        axios.get('http://localhost:3001/api/getreports', {
            headers: {
                Authorization: `Bearer ${token}`
            }
            ,
    ...
    function ignore() {
        ...
        axios.post('http://localhost:3001/api/reportignore', {
            headers: {
                Authorization: `Bearer ${token}`
            }
    ...
    function deletepost(post, event) {
        const token = localStorage.getItem('token')
        axios.post('http://localhost:3001/api/deletepost', {
            headers: {
                Authorization: `Bearer ${token}`
            }, params: {
                name: name,
