...
export default function JoinReqeusts(props) {
    ...
    useEffect(() => {
        ...
        axios.get('https://greddiit-backend-l5kk.onrender.com/api/JoinRequests', {
            headers: {
    ...
    function handleAccept(username,event){
        const token = localStorage.getItem('token')
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/joinrequests/accept', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params:{
    ...
    function handleReject(username, event){
        const token = localStorage.getItem('token')
        axios.post('https://greddiit-backend-l5kk.onrender.com/api/joinrequests/reject', {
            headers: {
                Authorization: `Bearer ${token}`
            },
            params:{
